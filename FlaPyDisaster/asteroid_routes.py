...
import math
from general import unit_conversions, general_objects, general_colors
import os
...
def asteroid_input_params_form():
    ...
    diameter_m = unit_conversions.distance_conversion(float(diameter_in), diameter_unit,
                                                      unit_conversions.DistanceUnits.meter)
    velocity_mps = unit_conversions.velocity_conversion(float(velocity_in), velocity_unit,
                                                        unit_conversions.VelocityUnits.mps)
    radius_obs_m = unit_conversions.distance_conversion(float(radius_obs_in), radius_obs_unit,
                                                        unit_conversions.DistanceUnits.meter)

    # create asteroid event from input parameters
    latlon_grid = general_objects.LatLonGrid(30, 20, 10, 30, 2, 2)

    # is this correct? Global variable
    global event
    event = asteroid_event.AsteroidEvent(diameter_m, angle_rad, velocity_mps, density_kgpm3, target_density_kgpm3,
                                         latlon_grid, (25, 20))
