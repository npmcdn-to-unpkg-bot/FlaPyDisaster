...
def breakup_altitude(impactor_density_kgpm3, diameter_m, velocity_mps, angle_rad):
    ...
    If = max(If_term(impactor_density_kgpm3, diameter_m, velocity_mps, angle_rad), 0)

    z_star = 0
    if If <= 1:
        z_star = -H * (math.log(Yi / (velocity_mps * velocity_mps)) + 1.308 - (0.314 * If) - (1.303 * math.sqrt(1 - If)))
...
def If_term(impactor_density_kgpm3, diameter_m, velocity_mps, angle_rad):
    """
